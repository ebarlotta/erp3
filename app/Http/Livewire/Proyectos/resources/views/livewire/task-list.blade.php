    ...
    <div class="space-y-2">
        @forelse($tasks as $task)
            <div class="bg-slate-800 rounded-lg p-4 border border-slate-700 flex items-center gap-4">
                <!-- Checkbox -->
                <button 
                    wire:click="updateStatus($task, '{{ $task->status === 'completed' ? 'pending' : 'completed' }}')"
                    class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors
                        {{ $task->status === 'completed' ? 'bg-green-500 border-green-500' : 'border-slate-600 hover:border-slate-400' }}"
                >
                ...
                <!-- Priority -->
                <select 
                    wire:change="updatePriority($task, $event.target.value)"
                    class="bg-slate-900 border border-slate-700 rounded px-2 py-1 text-xs text-white"
                >
                ...
                <!-- Delete -->
                <button wire:click="deleteTask($task)" class="text-slate-500 hover:text-red-400">
